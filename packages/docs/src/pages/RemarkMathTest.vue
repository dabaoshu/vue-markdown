  ...
  <div class="ast-ui-test-page">
    <header class="ast-ui-test-page__header">
      ...
      <div class="ast-ui-test-page__actions">
        ...
        <RouterLink class="btn btn--ghost" to="/demo?tab=math">打开 Demo Math Tab</RouterLink>
      </div>
    </header>
    ...
    <div class="ast-ui-test-page__body">
      ...
      <section v-if="selectedCase" class="ast-ui-test-page__detail">
        ...
        <div class="detail-flags">
          <span class="flag">math: {{ selectedCase.mathEnabled === false ? 'off' : 'on' }}</span>
        ...
        <div class="detail-grid">
          ...
          <div class="panel">
            <div class="panel__head">
              <span>实时预览</span>
              <span class="panel__hint">按本用例 math 开关渲染</span>
            </div>
            <div class="panel__preview">
              <RemarkMathPreview
                :key="selectedCase.id + '-selected'"
                :source="selectedCase.markdown"
                :math-enabled="selectedCase.mathEnabled !== false"
              />
            </div>
          </div>
        </div>
    ...
    <div class="ui-batch" aria-hidden="true">
      ...
      >
        <RemarkMathPreview
          ...
          :math-enabled="item.mathEnabled !== false"
        />
      </div>
    </div>
  </div>
...
import RemarkMathPreview from '@/test/remark-math/RemarkMathPreview';
import {
  REMARK_MATH_CASES,
  assertRemarkMathUi,
  ...
  treeToDisplayJson,
  type RemarkMathCaseGroup,
  type RemarkMathCaseResult
} from '@/test/remark-math';
import '@/test/_shared/ast-ui-test-page.scss';

const GROUP_LABELS: Record<RemarkMathCaseGroup | 'all', string> = {
  all: '全部',
  inline: '行内',
  block: '块级',
  mixed: '混排',
...
const cases = REMARK_MATH_CASES;
const activeGroup = ref<RemarkMathCaseGroup | 'all'>('all');
const selectedId = ref(cases[0]?.id ?? '');
const results = reactive<Record<string, RemarkMathCaseResult>>({});
...
const groupFilters = computed(() => {
  ...
  return (Object.keys(GROUP_LABELS) as Array<RemarkMathCaseGroup | 'all'>).map(
    (value) => ({
      value,
 ...
 * @param result 用例结果
 */
function resultGlyph(result?: RemarkMathCaseResult): string {
  if (!result) return '·';
  return result.ok ? '✓' : '✗';
}
...
async function rerunAll() {
  ...
  for (const testCase of cases) {
    results[testCase.id] = evaluateRemarkMathCase(testCase);
  }
  await nextTick();
  ...
  for (const testCase of cases) {
    ...
    try {
      assertRemarkMathUi(host, testCase);
