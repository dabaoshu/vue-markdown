  ...
  <div class="ast-ui-test-page">
    <header class="ast-ui-test-page__header">
      ...
      <div class="ast-ui-test-page__actions">
        ...
        <RouterLink class="btn btn--ghost" to="/demo?tab=code">打开 Demo Code Tab</RouterLink>
      </div>
    </header>
    ...
    <div class="ast-ui-test-page__body">
      ...
      <section v-if="selectedCase" class="ast-ui-test-page__detail">
        ...
        <div class="detail-flags">
          <span class="flag">highlight: {{ selectedCase.highlight === false ? 'off' : 'on' }}</span>
...
import CodeHighlightPreview from '@/test/code-highlight/CodeHighlightPreview';
import {
  ...
  treeToDisplayJson,
  type CodeHighlightCaseGroup,
  type CodeHighlightCaseResult
} from '@/test/code-highlight';
import '@/test/_shared/ast-ui-test-page.scss';

const GROUP_LABELS: Record<CodeHighlightCaseGroup | 'all', string> = {
  all: '全部',
  fence: '围栏',
  lang: '语言',
  inline: '行内',
...
const cases = CODE_HIGHLIGHT_CASES;
const activeGroup = ref<CodeHighlightCaseGroup | 'all'>('all');
const selectedId = ref(cases[0]?.id ?? '');
const results = reactive<Record<string, CodeHighlightCaseResult>>({});
...
const groupFilters = computed(() => {
  ...
  return (Object.keys(GROUP_LABELS) as Array<CodeHighlightCaseGroup | 'all'>).map(
    (value) => ({
      value,
 ...
 * @param result 用例结果
 */
function resultGlyph(result?: CodeHighlightCaseResult): string {
  if (!result) return '·';
  return result.ok ? '✓' : '✗';
}
...
async function rerunAll() {
  ...
  for (const testCase of cases) {
    results[testCase.id] = evaluateCodeHighlightCase(testCase);
  }
  await nextTick();
  ...
  for (const testCase of cases) {
    ...
    try {
      assertCodeHighlightUi(host, testCase);
