  ...
  <div class="ast-ui-test-page">
    <header class="ast-ui-test-page__header">
      <div class="ast-ui-test-page__intro">
        <h1>remark-http-resource 测试用例</h1>
        <p>
      ...
      <div class="ast-ui-test-page__actions">
        ...
        <RouterLink class="btn btn--ghost" to="/demo">打开 Demo</RouterLink>
      </div>
    </header>
    ...
    <div class="ast-ui-test-page__body">
      ...
      <section v-if="selectedCase" class="ast-ui-test-page__detail">
        ...
        <div class="detail-flags">
          ...
          <span class="flag">promoteBareUrls: {{ selectedCase.options?.promoteBareUrls ? 'on' : 'off' }}</span>
        ...
        <div class="detail-grid">
          ...
          <div class="panel">
            <div class="panel__head">
              <span>实时预览</span>
              <span class="panel__hint">按本用例 gfm / plugin / options 渲染</span>
            </div>
...
import RemarkHttpResourcePreview from '@/test/remark-http-resource/RemarkHttpResourcePreview';
import {
  ...
  treeToDisplayJson,
  type RemarkHttpResourceCaseGroup,
  type RemarkHttpResourceCaseResult
} from '@/test/remark-http-resource';
...
const cases = REMARK_HTTP_RESOURCE_CASES;
const activeGroup = ref<RemarkHttpResourceCaseGroup | 'all'>('all');
const selectedId = ref(cases[0]?.id ?? '');
const results = reactive<Record<string, RemarkHttpResourceCaseResult>>({});
...
const groupFilters = computed(() => {
  ...
  return (Object.keys(GROUP_LABELS) as Array<RemarkHttpResourceCaseGroup | 'all'>).map(
    (value) => ({
      value,
 ...
 * @param result 用例结果
 */
function resultGlyph(result?: RemarkHttpResourceCaseResult): string {
  if (!result) return '·';
  return result.ok ? '✓' : '✗';
}
...
async function rerunAll() {
  ...
  for (const testCase of cases) {
    results[testCase.id] = evaluateRemarkHttpResourceCase(testCase);
  }
  ...
  for (const testCase of cases) {
    ...
    try {
      assertRemarkHttpResourceUi(host, testCase);
