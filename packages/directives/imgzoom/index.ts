...
class ImgZoom {
  ...
  constructor({
    ...
    this.clickHandler = (e) => {
      ...
      if (showLoading) {
        loader = options?.customLoading
          ? options.customLoading()
          : createIdDom('div', '__imgZoomLoader');
      ...
      if (this.group && groupMap[this.group]) {
        ...
        const footer = createIdDom(
          'div',
          '__imgZoomFooter',
          `${index + 1} / ${groupMap[this.group].length}`
        );
        ...
        leftBtn.addEventListener('click', (e) => {
          if (index > 0) {
            ...
            footer.innerText = `${index + 1} / ${
              groupMap[this.group as string].length
            }`;
            if (title) {
              title.innerText = `${
                groupMap[this.group as string][index].title
              }`;
            }
          }
          e.stopPropagation();
  ...
  destory() {
    ...
    if (this.group && this.groupIndex && this.groupIndex >= 0) {
      const index = groupMap[this.group].findIndex(
        (item) => (item.imgSrc = this.imgSrc)
      );
      if (~index) {
...
export const ImgZoomDirective: ObjectDirective = {
  // 这个对象用于创建一个Vue.js指令，用于处理img-zoom属性的行为。
  mounted,
  unmounted,
  // inserted: mounted,
  // unbind: unmounted,
