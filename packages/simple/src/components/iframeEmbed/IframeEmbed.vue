...
<script lang="ts" setup>
  ...
  import { postIframeParams } from './postMessageBridge';
  import {
  ...
  const props = withDefaults(
    defineProps<{
      ...
      allow?: string;
      loadingText?: string;
      /** 加载中是否隐藏 iframe 避免白屏闪烁 */
      hideFrameWhileLoading?: boolean;
      /** iframe load 后是否自动 postMessage */
      autoPostMessage?: boolean;
      /** params 变化时无感更新（不 reload） */
      silentParamUpdate?: boolean;
      /** 非无感更新时 reload 是否显示 loading */
