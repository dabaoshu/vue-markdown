...
export class ProcessManager {
  ...
  /**
   * 切换目标目录（本仓库目录名、packages/xxx、或任意绝对路径）
  ...
  async start(name: string): Promise<ScriptRuntime> {
    ...
    this.appendLog(
      slot,
      'system',
      `[system] 在 ${this.getTarget().relativePath} 启动 pnpm run ${name}\n`
    );
...
function spawnChildScript(name: string, cwd: string): ChildProcess {
  ...
  if (process.platform === 'win32') {
    return spawn(
      'cmd.exe',
      ['/d', '/s', '/c', `chcp 65001>nul && pnpm run ${name}`],
      {
        cwd,
        env,
...
/**
 * 扫描 packages 下含 package.json 的目录
 */
function scanWorkspacePackages(): PackageTarget[] {
  ...
  for (const dir of dirs) {
    try {
      result.push(buildTargetFromPath(path.join(PACKAGES_DIR, dir), 'workspace'));
    } catch {
      // 忽略无效目录
    }
  }
...
/**
 * 解析用户输入为目标：本仓库短名 / packages/xxx / 绝对路径 / 已登记外部路径
 * @param input 输入
 */
function resolveTargetInput(input: string): PackageTarget {
  ...
  // 3) 本仓库 packages 短名或 packages/xxx
...
function resolveInitialTarget(): PackageTarget {
  ...
  try {
    ...
    if (packages.length === 0) {
      throw new Error('没有可用的目标工程（packages 为空且未添加外部路径）');
    }
    return packages[0];
  }
}
